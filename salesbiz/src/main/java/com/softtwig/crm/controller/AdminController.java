...
public class AdminController extends ControllerUtils implements Serializable {
	...
	public String assignprivilegeRole(@ModelAttribute("roletype") RoleBO rolebo, HttpServletRequest request) {
		 ...
		 * if(null!=privilegebolist&&privilegebolist.size()>0){ for(PrivilegesBO bo :
		 * privilegebolist){ RoleBO role = new RoleBO();
		 * role.setPrivilegeId(bo.getPrivilegeId());
		 * role.setPrivilegename(bo.getPrivilegename());
		 * role.setRoleName(borole.getRoleName()); role.setRoleId(borole.getRoleId());
		 * listrolebo.add(role); } }
		 */
		final String paging = request.getParameter("page");
		if (null != paging) {
	...
	public String editRolePrivilege(Model model, HttpServletRequest request) {
		...
		/*
		 * if(null!=request.getParameter("roleId")&&null!=request.getParameter(
		 * "privilegeid")){ long roleid = Long.parseLong(id); long privilegeid =
		 * Long.parseLong(id2); rolebo.setRoleId(roleid);
		 * rolebo.setPrivilegeId(privilegeid); rolebo =
		 * adminService.getroleprivilege(rolebo);
		 * 
		 * }
		 */ List<RoleBO> rolebolist = new ArrayList<RoleBO>();
		rolebolist = roletypeservice.viewrole();
	...
	public String editRolePrivileges(@ModelAttribute("rolebo") RoleBO rolebo) {
		 ...
		 * role.setPrivilegeId(bo.getPrivilegeId());
		 * role.setPrivilegename(bo.getPrivilegename());
		 * role.setRoleName(rolebo.getRoleName()); role.setRoleId(rolebo.getRoleId());
		 * listrolebo.add(role); } }
		 */
	...
	public String updatePrivilegeAccess(@Valid Model model,
		...
		// ArrayList<>(Arrays.asList(privilegeBO.getAccessName().split(",")));
	...
	/*
	 * public void getuserrole(Model model){ List<UserRolesBO> listbo = new
	 * ArrayList<UserRolesBO>(); listbo=userroleService.retriveUserRoles(listbo);
	 * model.addAttribute("listbo", listbo); }
	 */
	@PreAuthorize("hasRole('ADMIN')")
	@RequestMapping(value = "/create-campaign", method = RequestMethod.GET)
	public String createcampaign(Model model, HttpServletRequest request) throws MySalesException, MalformedURLException,
	NumberFormatException, FileNotFoundException, SerialException, SQLException {
		...
		adminLoginBO.setFirstName(getUserSecurity().getName());
		model.addAttribute("userType", adminLoginBO);
	    CampaignsDO campaigns = new CampaignsDO();
		int page = 1;
		final String paging = request.getParameter("page");
		if (null != paging) {
			page = Integer.parseInt(paging);
